...
## ⚙️ Configuração CORS (ALLOWED_ORIGINS)

Para conectar o front hospedado no GitHub Pages ou outro domínio, configure a variável de ambiente `ALLOWED_ORIGINS` no painel do Render.

- Formato: lista separada por vírgula. Exemplo:

```
ALLOWED_ORIGINS=https://seuusuario.github.io,https://outro.dominio.com
```

- Para testes rápidos você pode usar `*`, mas não é recomendado em produção:

```
ALLOWED_ORIGINS=*
```

- Após alterar a variável, faça redeploy do serviço.

No front (GitHub Pages) você pode definir a URL do backend dinamicamente adicionando antes do `interact.js` no HTML:

```html
<script>
  window.BACKEND_URL = 'https://SEU_BACKEND_ON_RENDER/agendamento-online.php';
</script>
<script src="interact.js" defer></script>
```

Isso evita editar os arquivos JS diretamente e facilita múltiplos ambientes (dev/staging/prod).